...
| Validation script changes | `R2` | Allowed when offline | Owner approval if risk expands | PR | Local validation, CI | Source revert | Live calls, secret reads, deployment, or rule weakening | Tier 1 only after review |
| CI workflow changes | `R2` | Blocked by default for auto-merge | Yes unless purely mechanical and safe | PR | CI result, diff review | Source revert | Any secret, deploy, write, or paid path | Owner review |
| Authority, safe-merge, approval workflow, owner record, or Constitution change | `R2` | Blocked for auto-merge | Yes | PR and audit note after audit exists | Reason, risk, validation, CI | Source revert | Missing owner approval | Blocked |
