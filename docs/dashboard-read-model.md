...
The read model is deterministic and generated from source-controlled records. The generated `dashboard/dashboard-data.js` file is ignored rather than committed; CI can build it, and every coordinator start regenerates it before serving the dashboard. Generation does not call live services.
...
`dashboard/dashboard-data.js` is generated output and must not be committed. Build it for local static viewing with:

```powershell
npm.cmd run dashboard:build
```

`npm.cmd run dashboard:check` validates the source records and dashboard invariants directly without trusting an old generated artifact. `npm.cmd run live:start` and the production container regenerate the file before the server starts.
